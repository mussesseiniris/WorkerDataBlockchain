                    ...
                    <ul className="flex flex-col gap-3">
                        {unread.map(n => {
                            ...
                            <li
                                ...
                                className="flex items-start gap-4 px-5 py-4 bg-white border border-gray-100 rounded-xl shadow-sm hover:shadow-md hover:border-gray-200 cursor-pointer transition-all"
                            >
                                <NotificationIcon type={n.notificationType} size="md" />
                                <div className="flex-1 min-w-0">
                                    ...
                                    {n.workerInfoDesc && (
                                        <p className="text-sm text-gray-600 mt-1 break-words">{n.workerInfoDesc}</p>
                                    )}
                                </div>
                                <p className="text-xs text-gray-400 whitespace-nowrap flex-shrink-0 mt-0.5">{formatRelativeTime(n.notificationTime)}</p>
                            </li>
                            );
                        })}
                    </ul>
                ...
                {read.length === 0 ? (
                    ...
                    <ul className="flex flex-col gap-3">
                        {read.map(n => {
                            const typeStyle = getTypeStyle(n.notificationType);
                            return (
                            <li key={n.id} className="flex items-start gap-4 px-5 py-4 bg-gray-50 border border-gray-100 rounded-xl opacity-75">
                                <NotificationIcon type={n.notificationType} size="md" />
                                <div className="flex-1 min-w-0">
                                    ...
                                    {n.workerInfoDesc && (
                                        <p className="text-sm text-gray-400 mt-1 break-words">{n.workerInfoDesc}</p>
                                    )}
                                </div>
                                <p className="text-xs text-gray-300 whitespace-nowrap flex-shrink-0 mt-0.5">{formatRelativeTime(n.notificationTime)}</p>
                            </li>
                            );
                        })}
                    </ul>
